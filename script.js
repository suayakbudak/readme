...
// Geri dönüşte animasyon atlanır; sayfa yenilenince çalışır
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Geri/ileri navigasyonda veya Projects'ten dönüşte animasyon atla
    const nav = performance.getEntriesByType?.('navigation')?.[0];
    const isBackForward = nav?.type === 'back_forward';
    const fromProjects = document.referrer && (document.referrer.includes('projects.html') || document.referrer.includes('/projects'));
    const skipAnimations = isBackForward || fromProjects;

    // Ses efektleri - hem animasyon hem skip durumunda çalacak
    ...
    function playLampSounds() {
        lampOnSound.play().catch(() => {});
    ...
    function skipToFinalState() {
        ...
        if (readme) {
            readme.classList.add('visible', 'risen', 'slid-right');
        }
    ...
    // Geri dönüşte typing animasyonu çalışmasın
    if (!skipAnimations) {
